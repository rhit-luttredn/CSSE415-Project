...
if __name__ == "__main__":
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, i, args.capture_f, run_name, args.gamma) for i in range(args.num_envs)]
    )
    ...
    agent.eval()
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, i, args.capture_f, f"{args.exp_name}/{run_name}-eval") for i in range(args.num_envs)],
    )
    eval_episodes = 10
    obs, _ = envs.reset()
    episodic_returns = []
