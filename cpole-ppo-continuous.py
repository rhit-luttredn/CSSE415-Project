...
def make_env(idx, capture_video, run_name, gamma):
    def thunk():
        if capture_video and idx == 0:
...
if __name__ == "__main__":
    ...
    run_name = f"ContinuousCartPole__{args.exp_name}__{args.seed}__{int(time.time())}"
    ...
    # )
    envs = gym.vector.SyncVectorEnv(
        [make_env(i, args.capture_f, run_name, args.gamma) for i in range(args.num_envs)]
    )
    ...
    # )
    envs = gym.vector.SyncVectorEnv(
        [make_env(i, args.capture_f, f"{args.exp_name}/{run_name}-eval") for i in range(args.num_envs)],
    )
    eval_episodes = 10
    obs, _ = envs.reset()
    episodic_returns = []
